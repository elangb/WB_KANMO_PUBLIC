...
select event,datetime,uniqueid as jumlah,1 Seconds from qstats.queue_stats_full where (queue='60012' or queue='60011') 
union
select disposition as event,calldate,uniqueid as jumlah,0 AS seconds from asteriskcdrdb.cdr where dst in ('60012','60011')
UNION 
select 'NEWANSWERED' as event,calldate,uniqueid as jumlah,billsec AS seconds from asteriskcdrdb.cdr where dst in ('60012','60011')
union
select 'CONNECTA' as event,calldate,uniqueid as jumlah,0 AS seconds from asteriskcdrdb.cdr where dst in ('60012','60011')
union
...
union
select a.event,a.datetime,a.uniqueid as jumlah,(SELECT g.duration FROM asteriskcdrdb.cdr g WHERE g.uniqueid = a.uniqueid and g.disposition='ANSWERED' ORDER BY uniqueid DESC LIMIT 1) AS Seconds from qstats.queue_stats_full a where a.qname in ('2','3') and a.event<>'RINGNOANSWER'
union
...
datetime !='' and labelreport !='' AND (datetime BETWEEN '2024-05-15 00:00:00' AND '2024-05-15 23:59:59')
GROUP BY
